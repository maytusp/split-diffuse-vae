...
from models.diffusion.refine_wrapper import DDPMRefine
from util import space_timesteps


class splitFinetuneDDPMWrapper(pl.LightningModule):
    def __init__(
        self,
        split_vae,
        target_network,
    ...
    # def get_vae(self, split_vae=None):
    #     self.split_vae = split_vae
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.conditional:
            x = batch
            x_aux, _ = self.split_vae.scramble(batch)
    
            with torch.no_grad():
                mu, logvar = self.split_vae.encode(x * 0.5 + 0.5)
                mu_aux, logvar_aux = self.split_vae.encode_aux(x_aux* 0.5 + 0.5)

                z_main = self.split_vae.reparameterize(mu, logvar)
                z_aux = self.split_vae.reparameterize(mu_aux, logvar_aux)
                z = torch.cat((z_main,z_aux), dim=1)
                cond = self.split_vae.decode(z)
                cond = 2 * cond - 1

                cond_aux = self.split_vae.decode_aux(z_aux)
                cond_aux = 2 * cond_aux - 1
            ...
            if torch.rand(1)[0] < self.cfd_rate:
                ...
                cond_aux = torch.zeros_like(x)
                z_aux = torch.zeros_like(z_aux)                
        else:
            x = batch
    ...
    def predict_step(self, batch, batch_idx, dataloader_idx=None):
        ...
        if self.eval_mode == "sample":
            ...
            recons = img #self.split_vae.forward_recons(img * 0.5 + 0.5)
            #recons = 2 * recons - 1

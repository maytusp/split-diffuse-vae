...
class splitDDPMWrapper(pl.LightningModule):
    def __init__(
        self,
        online_network,
        target_network,
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.conditional:
            x = batch
            x_aux, _ = self.vae.scramble(batch)
    
            with torch.no_grad():
                mu, logvar = self.vae.encode(x * 0.5 + 0.5)
                mu_aux, logvar_aux = self.vae.encode_aux(x_aux* 0.5 + 0.5)

                z_main = self.vae.reparameterize(mu, logvar)
                z_aux = self.vae.reparameterize(mu_aux, logvar_aux)
                z = torch.cat((z_main,z_aux), dim=1)
                cond = self.vae.decode(z)
                cond = 2 * cond - 1

                cond_aux = self.vae.decode_aux(z_aux)

...
from models.diffusion import DDPM, DDPMv2, splitDDPMWrapper, SuperResModel
...
def generate_recons(config):
    ...
    ddpm_wrapper = splitDDPMWrapper.load_from_checkpoint(
    ...
    folder_list = os.listdir(config_ddpm.data.root)
    for folder_idx, folder in enumerate(folder_list):
        # Dataset
        root = os.path.join(config_ddpm.data.root, folder)
        save_path = os.path.join(config_ddpm.evaluation.save_path, folder)
        ...
        dataset_ordered = Subset(dataset, indices=[i for i in range(len_dataset)])
        # Predict loader
        ...
        test_kwargs["default_root_dir"] = save_path

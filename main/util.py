...
def get_dataset(name, root, image_size, norm=True, flip=False, **kwargs):
    ...
    if name == "miniimagenet" or name == "clevr":
        t_list = [T.Resize((int(image_size*1.5), int(image_size*1.5))), T.CenterCrop(image_size)]
    else:
    ...
    if name == "celeba":
        ...
        dataset = AFHQv2Dataset(root, animal="dog", norm=norm, transform=transform, **kwargs)
    elif name == "afhq_cat":
        dataset = AFHQv2Dataset(root, animal="cat", norm=norm, transform=transform, **kwargs)    
    elif name == "afhq_wild":
        dataset = AFHQv2Dataset(root, animal="wild", norm=norm, transform=transform, **kwargs)                
    elif name == "ffhq":
        dataset = FFHQDataset(root, norm=norm, transform=transform, **kwargs)
    elif name  == "miniimagenet":
        dataset = miniImageNet(root, norm=norm, transform=transform, **kwargs)
    elif name  == "clevr":
        dataset = clevr(root, norm=norm, transform=transform, **kwargs)
    elif name  == "carla":
        dataset = carla(root, norm=norm, transform=transform, **kwargs)                
    elif name == "cifar10":

...
from models.diffusion import DDPM, DDPMv2, DDPMRefine, splitFinetuneDDPMWrapper, SuperResModel, UNetModel
from models.vae import VAE as dummyVAE
from models.split_vae import VAE as splitVAE
...
def train(config):
    ...
    # Dummy VAE is just for loading DDPMWrapper
    # Because DDPM's weights can't be loaded without VAE weights
    vae = dummyVAE.load_from_checkpoint(
        config.training.dummy_vae_chkpt_path,
        input_res=image_size,
    )


    # Load VAE that we will use
    split_vae = splitVAE.load_from_checkpoint(
    ...
    # ddpm_wrapper = DDPMRefine.load_from_checkpoint(
    #     config.training.restore_path,
    ...
    # Fuse pretrained split-vae and pretrained ddpm
    # fusion_model = ddpm_wrapper
    fusion_model = splitFinetuneDDPMWrapper(
        split_vae,
        online_network=ddpm_wrapper.online_network,
        target_network=ddpm_wrapper.target_network,
        vae=ddpm_wrapper.vae,
        lr=lr,
        cfd_rate=config.training.cfd_rate,
        n_anneal_steps=config.training.n_anneal_steps,
        loss=config.training.loss,
    ...
    trainer.fit(fusion_model, train_dataloader=loader)
